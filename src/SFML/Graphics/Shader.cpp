...
Shader::~Shader()
{
    ...
    if (m_shaderProgram)
        glCheck(GLEXT_glDeleteObject(m_shaderProgram));
}
...
void Shader::setParameter(const std::string& name, float x)
{
    if (m_shaderProgram)
    {
        ...
        glCheck(GLEXT_glUseProgramObject(m_shaderProgram));
...
void Shader::bind(const Shader* shader)
{
    ...
    {
        // Enable the program
        glCheck(GLEXT_glUseProgramObject(shader->m_shaderProgram));
...
{
    static bool vertexAvailable = false;
    static bool fragmentAvailable = false;
    static bool geometryAvailable = false;
    ...
    if (!checked)
    {
        ...
        vertexAvailable = GLEXT_multitexture         &&
                          GLEXT_fragment_program     &&
                          GLEXT_shading_language_100 &&
                          GLEXT_shader_objects       &&
                          GLEXT_vertex_shader        &&
                          GLEXT_fragment_shader;
        fragmentAvailable = vertexAvailable;
        geometryAvailable = vertexAvailable && GLEXT_geometry_shader4;

        checked = true;
    }

    if ((shaderTypes & Vertex) && !vertexAvailable)
        return false;

    if ((shaderTypes & Fragment) && !fragmentAvailable)
        return false;

    if ((shaderTypes & Geometry) && !geometryAvailable)
        return false;
...
bool Shader::compile(const char* vertexShaderCode, const char* geometryShaderCode, const char* fragmentShaderCode)
{
    ...
    // Create the program
    m_shaderProgram = glCheck(GLEXT_glCreateProgramObject());
    ...
    if (vertexShaderCode)
    {
        ...
        glCheck(GLEXT_glAttachObject(m_shaderProgram, vertexShader));
        glCheck(GLEXT_glDeleteObject(vertexShader));
    }
    ...
    if (geometryShaderCode)
    {
        ...
        // Attach the shader to the program, and delete it (not needed anymore)
        glCheck(GLEXT_glAttachObject(m_shaderProgram, geometryShader));
        glCheck(GLEXT_glDeleteObject(geometryShader));
    }
    ...
    if (fragmentShaderCode)
    {
        ...
        // Attach the shader to the program, and delete it (not needed anymore)
        glCheck(GLEXT_glAttachObject(m_shaderProgram, fragmentShader));
        glCheck(GLEXT_glDeleteObject(fragmentShader));
    }

    // Link the program
    glCheck(GLEXT_glLinkProgram(m_shaderProgram));

    // Check the link log
    GLint success;
    glCheck(GLEXT_glGetObjectParameteriv(m_shaderProgram, GLEXT_GL_OBJECT_LINK_STATUS, &success));
    if (success == GL_FALSE)
    {
        char log[1024];
        glCheck(GLEXT_glGetInfoLog(m_shaderProgram, sizeof(log), 0, log));
...
int Shader::getParamLocation(const std::string& name)
{
    ...
    else
    {
        ...
        int location = GLEXT_glGetUniformLocation(m_shaderProgram, name.c_str());
